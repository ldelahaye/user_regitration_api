...
async def test_register_user_returns_201(client: AsyncClient, mock_user_service: AsyncMock) -> None:
    response = await client.post("/users", json={"email": "test@example.com", "password": "securepassword123"})
    ...
    mock_user_service.register.assert_called_once_with("test@example.com", "securepassword123")
...
async def test_register_user_duplicate_email_returns_409(client: AsyncClient, mock_user_service: AsyncMock) -> None:
    ...
    response = await client.post("/users", json={"email": "taken@example.com", "password": "securepassword123"})
...
async def test_register_user_invalid_email_returns_422(client: AsyncClient, mock_user_service: AsyncMock) -> None:
    response = await client.post("/users", json={"email": "not-an-email", "password": "securepassword123"})
...
async def test_register_user_short_password_returns_422(client: AsyncClient, mock_user_service: AsyncMock) -> None:
    response = await client.post("/users", json={"email": "test@example.com", "password": "short"})
